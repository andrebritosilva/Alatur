...
//------------------------------------------------------------------------------------------
/*/{Protheus.doc} TURI023

Função de integração do Cadastro de Categorias de Veículo 
Mensagem única - DirectionType

@sample	TURI023(cXml, cTypeTrans, cTypeMessage)
@param		cXml - String com o XML recebido pelo EAI Protheus
			cType - String com o Tipo de transação
...
/*/
//------------------------------------------------------------------------------------------
Function TURI023(cXml, nTypeTrans, cTypeMessage)

Local lRet		  := .T. 
Local cEvento   := 'upsert'
Local cAdapter  := 'TURA023'
Local cMsgUnica := 'DirectionType'
Local cMarca	  := 'PROTHEUS'
Local cAlias	  := 'G50'
Local cCampo	  := 'G50_CODIGO'
...
Local nX
Local lDelete
Local lMsblql	:= AllTrim(GetSx3Cache("G50_MSBLQL", "X3_CAMPO")) == "G50_MSBLQL"  //FieldPos('G50_MSBLQL') > 0
...
Do Case
	Case nTypeTrans == TRANS_SEND
		...
		cCodeInt := oModel:GetValue('MASTER', 'G50_CODIGO')
		cDesc	  := oModel:GetValue('MASTER', 'G50_DESCR')
		If lMsblql
			cBlocked := oModel:GetValue('MASTER', 'G50_MSBLQL')
		Endif
		cIntID	  := TURXMakeId(cCodeInt, 'G50')
	...
	Case nTypeTrans == TRANS_RECEIVE .And. oXML:Parse(cXml)
		Do Case
			...
			Case (cTypeMessage == EAI_MESSAGE_BUSINESS)
				...
				cCodeInt	:= PadR(AllTrim(TURXRetId(cMarca, cAlias, cCampo, cExtID, @cIntID, 3)), TamSx3('G50_CODIGO')[1])
				
				If Upper(cEvent) == 'UPSERT'
					If !Empty(cIntID) .And. G50->(DbSeek(xFilial('G50') + cCodeInt))
					...
					If oModel:Activate()
						...
						If cEvent <> MODEL_OPERATION_DELETE
							If cEvent == MODEL_OPERATION_INSERT
								oModelCab:SetValue('G50_CODIGO', cCodeInt)
							Endif
							oModelCab:SetValue('G50_DESCR' , cDesc)
							If lMsblql
								oModelCab:SetValue('G50_MSBLQL', TURXLogic(cBlocked, TP_CHAR1_RET))
							Endif
						Endif
