...
//------------------------------------------------------------------------------------------
/*/{Protheus.doc} TURI013

Função de integração do Cadastro de Planos de Seguro 
Mensagem única - InsurancePlan

@sample	TURI013(cXml, cTypeTrans, cTypeMessage)
...
/*/
//------------------------------------------------------------------------------------------
Function TURI013(cXml, nTypeTrans, cTypeMessage)

Local lRet      := .T. 
Local cEvento   := 'upsert'
Local cAdapter  := 'TURA013'
Local cMsgUnica := 'InsurancePlan'
Local cMarca    := 'PROTHEUS'
Local cAlias    := 'G4G'
Local cCampo    := 'G4G_CODIGO'
...
Local nX
Local lDelete
Local lMsblql	:= AllTrim(GetSx3Cache("G4G_MSBLQL", "X3_CAMPO")) == "G4G_MSBLQL" //FieldPos('G4G_MSBLQL')
...
Do Case
	Case nTypeTrans == TRANS_SEND
		...
		cCodeInt := Alltrim(oModel:GetValue('MASTER', 'G4G_CODIGO'))
		cNome    := Alltrim(oModel:GetValue('MASTER', 'G4G_NOME'))
		cFornInt := Alltrim(oModel:GetValue('MASTER', 'G4G_FORNEC'))
		cLojaInt := Alltrim(oModel:GetValue('MASTER', 'G4G_LOJA'))
		cDesc    := Alltrim(oModel:GetValue('MASTER', 'G4G_DESCR'))
		If lMsblql
			cBlocked := oModel:GetValue('MASTER', 'G4G_MSBLQL')
		Endif
		cIntID   := TURXMakeId(cCodeInt, 'G4G')
		...
		cXMLRet +=		'<VendorInternalId>' + IntForExt(,,cFornInt,cLojaInt)[2] + '</VendorInternalId>' //TURXMakeId(cFornInt + "|" + cLojaInt + '|F', 'SA2')
		If lMsblql
	...
	Case nTypeTrans == TRANS_RECEIVE .And. oXML:Parse(cXml)
		Do Case
			...
			Case (cTypeMessage == EAI_MESSAGE_BUSINESS)
				...
				cCodeInt := PadR(AllTrim(TURXRetId(cMarca, cAlias, cCampo  , cExtID  , @cIntID, 3)), TamSx3('G4G_CODIGO')[1])
				If (aForn := IntForInt(cFornExt,cMarca))[1]
					cFornInt := PadR(aForn[2][3], TamSx3('G4G_FORNEC')[1])
					cLojaInt := PadR(aForn[2][4], TamSx3('G4G_LOJA')[1])
				Else
					Return aForn 
				Endif
				...
				If Upper(cEvent) == 'UPSERT'
					If !Empty(cIntID) .And. G4G->(DbSeek(xFilial('G4G') + cCodeInt))
					...
					If oModel:Activate()
						...
						If cEvent <> MODEL_OPERATION_DELETE
							If cEvent == MODEL_OPERATION_INSERT
								oModelCab:SetValue('G4G_CODIGO', cCodeInt)
								oModelCab:SetValue('G4G_FORNEC', cFornInt)
								oModelCab:SetValue('G4G_LOJA'  , cLojaInt)
							Endif
							oModelCab:SetValue('G4G_NOME'  , cNome)
							oModelCab:SetValue('G4G_DESCR' , cDesc)
							If lMsblql
								oModelCab:SetValue('G4G_MSBLQL', TURXLOGIC(cBlocked, TP_CHAR1_RET))
							Endif
						Endif
