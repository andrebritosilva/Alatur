...
//------------------------------------------------------------------------------------------
/*/{Protheus.doc} TURI012

Função de integração do Cadastro de Tipos de Visto 
Mensagem única - VisaType

@sample	TURI012(cXml, cTypeTrans, cTypeMessage)
...
@author 	Thiago Tavars
@since		30/09/2015
@version 	P12.1.8
/*/
//------------------------------------------------------------------------------------------
Function TURI012(cXml, nTypeTrans, cTypeMessage)

Local lRet      := .T. 
Local cEvento   := 'upsert'
Local cAdapter  := 'TURA012'
Local cMsgUnica := 'VisaType'
Local cMarca    := 'PROTHEUS'
Local cAlias    := 'G4F'
Local cCampo    := 'G4F_CODIGO'
...
Local nX
Local lDelete
Local lMsblql	:=  AllTrim(GetSx3Cache("G4F_MSBLQL", "X3_CAMPO")) == "G4F_MSBLQL"//FieldPos('G4F_MSBLQL') > 0
...
Do Case
	Case nTypeTrans == TRANS_SEND
		...
		cCodeInt	:= oModel:GetValue('MASTER', 'G4F_CODIGO')
		cDesc		:= oModel:GetValue('MASTER', 'G4F_DESCR')
		If lMsblql
			cBlocked	:= oModel:GetValue('MASTER', 'G4F_MSBLQL')
		Endif
		cIntId		:= TURXMakeId(cCodeInt, 'G4F')
	...
	Case nTypeTrans == TRANS_RECEIVE .And. oXML:Parse(cXml)
		Do Case
			...
			Case ( cTypeMessage == EAI_MESSAGE_BUSINESS )
				...
				cCodeInt := PadR(AllTrim(TURXRetId(cMarca, cAlias, cCampo, cExtID, @cIntID, 3)), TamSx3('G4F_CODIGO')[1])
					
				If Upper(cEvent) == 'UPSERT'
					If !Empty(cIntID) .And. G4F->(DbSeek(xFilial('G4F') + cCodeInt))
						cEvent := MODEL_OPERATION_UPDATE
					Else
						cEvent   := MODEL_OPERATION_INSERT
						cCodeInt := GetSXENum('G4F', 'G4F_CODIGO')
						cIntID   := TURXMakeId(cCodeInt, 'G4F')
					Endif
					...
					If oModel:Activate()
						...
						If cEvent <> MODEL_OPERATION_DELETE
							If cEvent == MODEL_OPERATION_INSERT
								oModelCab:SetValue('G4F_CODIGO', cCodeInt)
							Endif
							oModelCab:SetValue('G4F_DESCR' , cDesc)
							If lMsblql
								oModelCab:SetValue('G4F_MSBLQL', TURXLogic(cBlocked, TP_CHAR1_RET))
							Endif
						Endif
