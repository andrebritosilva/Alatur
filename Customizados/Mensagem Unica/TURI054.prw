...
//------------------------------------------------------------------------------------------
/*/{Protheus.doc} TURI054

Função de integração do Cadastro de Motivos de Reembolso 
Mensagem única - RefundReason

@sample	TURI054(cXml, cTypeTrans, cTypeMessage)
...
/*/
//------------------------------------------------------------------------------------------
Function TURI054(cXml, nTypeTrans, cTypeMessage)

Local lRet      := .T. 
Local cEvento   := 'upsert'
Local cAdapter  := 'TURA054'
Local cMsgUnica := 'REFUNDREASON'
Local cMarca    := 'PROTHEUS'
Local cAlias    := 'G8P'
Local cCampo    := 'G8P_CODIGO'
...
Local nX
Local lDelete
Local lMsblql	:= AllTrim(GetSx3Cache("G8P_MSBLQL", "X3_CAMPO")) == "G8P_MSBLQL" //FieldPos("G8P_MSBLQL") > 0
...
Do Case
	Case nTypeTrans == TRANS_SEND
		...
		cCodeInt := oModel:GetValue('MASTER', 'G8P_CODIGO')
		cDesc    := oModel:GetValue('MASTER', 'G8P_DESCR')
		If lMsblql
			cBlocked := oModel:GetValue('MASTER', 'G8P_MSBLQL')
		Endif
		cIntID   := TURXMakeId(cCodeInt, 'G8P')
	...
	Case nTypeTrans == TRANS_RECEIVE .And. oXML:Parse(cXml)
		Do Case
			...
			Case (cTypeMessage == EAI_MESSAGE_BUSINESS)
				...
				cCodeInt := PadR(AllTrim(TURXRetId(cMarca, cAlias, cCampo, cExtID, @cIntID, 3)), TamSx3('G8P_CODIGO')[1])
				
				G8P->(DbSetOrder(1))
				If Upper(cEvent) == 'UPSERT'
					If !Empty(cIntID) .And. G8P->(DbSeek(xFilial('G8P') + cCodeInt))
					...
					If oModel:Activate()
						...
						If cEvent <> MODEL_OPERATION_DELETE
							If cEvent == MODEL_OPERATION_INSERT
								oModelCab:SetValue('G8P_CODIGO', cCodeInt)
							Endif
							oModelCab:SetValue('G8P_DESCR' , cDesc)
							If lMsblql
								oModelCab:SetValue('G8P_MSBLQL', TURXLogic(cBlocked, TP_CHAR1_RET))
							Endif
						Endif
