...
//------------------------------------------------------------------------------------------
/*/{Protheus.doc} TURI021

Função de integração do Cadastro de Categorias de Veiculo 
Mensagem única - VehicleCategory

@sample	TURI021(cXml, cTypeTrans, cTypeMessage)
...
/*/
//------------------------------------------------------------------------------------------
Function TURI021(cXml, nTypeTrans, cTypeMessage)

Local lRet      := .T. 
Local cEvento   := 'upsert'
Local cAdapter  := 'TURA021'
Local cMsgUnica := 'VEHICLECATEGORY'
Local cMarca    := 'PROTHEUS'
Local cAlias    := 'G4Y'
Local cCampo    := 'G4Y_CODIGO'
...
Local nX
Local lDelete
Local lMsblql	:= AllTrim(GetSx3Cache("G4Y_MSBLQL", "X3_CAMPO")) == "G4Y_MSBLQL" //FieldPos('G4Y_MSBLQL') > 0  
...
Do Case
	Case nTypeTrans == TRANS_SEND
		...
		cCodeInt := oModel:GetValue('MASTER', 'G4Y_CODIGO')
		cDesc    := oModel:GetValue('MASTER', 'G4Y_DESCR')
		If lMsblql
			cBlocked := oModel:GetValue('MASTER', 'G4Y_MSBLQL')
		Endif
		cIntID   := TURXMakeId(cCodeInt, 'G4Y')
		...
		Do Case
			...
			Case (cTypeMessage == EAI_MESSAGE_BUSINESS)
				...
				If Upper(cEvent) == 'UPSERT'
					If !Empty(cIntID) .And. G4Y->(DbSeek(xFilial('G4Y') + cCodeInt))
					...
					If oModel:Activate()
						...
						If cEvent <> MODEL_OPERATION_DELETE
							If cEvent == MODEL_OPERATION_INSERT
								oModelCab:SetValue('G4Y_CODIGO', cCodeInt)
							Endif
							oModelCab:SetValue('G4Y_DESCR' , cDesc)
							If lMsblql
								oModelCab:SetValue('G4Y_MSBLQL', TURXLogic(cBlocked, TP_CHAR1_RET))
							Endif
						Endif
