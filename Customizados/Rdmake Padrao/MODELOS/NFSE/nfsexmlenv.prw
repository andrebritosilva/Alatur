...
user function nfseXMLEnv( cTipo, dDtEmiss, cSerie, cNota, cClieFor, cLoja, cMotCancela,aAIDF )
			...
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				If !lAglutina .Or. Len(aProd) == 0
					...
					aadd(aProd,	{Len(aProd)+1,;
								...
								SB1->B1_UM,;
								(cAliasSD2)->D2_QUANT,;
								a410Arred( FunValUnit((cAliasSD2)->D2_TIPO, (cAliasSD2)->D2_PRCVEN, (cAliasSD2)->D2_QUANT,(cAliasSD2)->D2_VALICM), "D2_TOTAL"),; //IIF(!(cAliasSD2)->D2_TIPO$"IP",(cAliasSD2)->D2_PRCVEN,0),; // Valor unitário
								...
								a410Arred( FunValTot((cAliasSD2)->D2_TIPO,(cAliasSD2)->D2_PRCVEN, (cAliasSD2)->D2_QUANT, (cAliasSD2)->D2_TOTAL, (cAliasSD2)->D2_DESCON, (cAliasSD2)->D2_DESCZFR, (cAliasSD2)->D2_VALICM), "D2_TOTAL")+Iif(lAglutina,((cAliasSD2)->D2_DESCON+(cAliasSD2)->D2_DESCZFR),0),; //Valor Total
	...
	If !Empty(cString) .AND. !Empty(SuperGetMV("MV_DIRNFSE",,"")) 
			
		// VERIFICA SE ARQUIVO DE LOG FOI CRIADO
		If (nHandCre := FCREATE(SuperGetMV("MV_DIRNFSE") + "NFSE_"+AllTrim(xFilial("SF2")+"_"+cNota+"_"+cSerie+"_"+cClieFor+"_"+cLoja)+".TXT", 0)) > 0
			FWRITE( nHandCre, cString)
			FCLOSE(nHandCre)
		EndIf
