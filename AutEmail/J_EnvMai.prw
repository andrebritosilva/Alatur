...
User Function J_EnvMai()
	...
	If MAILTO->(!Eof())
		...
		While MAILTO->(!Eof())
			...
			If !Empty(MAILTO->ZM_TO)
				...
				If MailIs(Alltrim(MAILTO->ZM_CC), .T., .T.) .And. MailIs(Alltrim(MAILTO->ZM_TO), .T., .T.) 		
					...
					If !Empty((MAILTO->ZM_ATTACH))  
						...
						If ALLTRIM(MAILTO->ZM_BCC) == "CAR"
												
							aAnexo := SEPARA(MAILTO->ZM_ATTACH,";",.T.)

							For _i:= 1 to Len(aAnexo)
							
								If File(aAnexo[_i])	

									If oMessage:AttachFile(Alltrim(aAnexo[_i])) < 0
										Conout( "J_EnvMai: " + Dtos(Date()) + " - " + Time() + " - Erro ao ANEXAR o arquivo referente ao  R_E_C_N_O_  " + cValToChar(MAILTO->R_E_C_N_O_) + ".")
										RpcClearEnv() 
										Return .F.
									Else
										aNomArq:= SEPARA(aAnexo[_i],"\",.T.)	
										oMessage:AddAtthTag( 'Content-Disposition: attachment; filename="' + AllTrim(aNomArq[LEN(aNomArq)]) + '"')
									EndIf
			...
			If SZM->ZM_NRTENTA > nTenta .AND. SZM->ZM_NOTIFIC .AND. !SZM->ZM_ENVIADO .AND. SZM->ZM_EMISSAO == dDatabase 
				...
				If AllTrim(SZM->ZM_BCC) == "CAR"

					AADD(aExcCAR,{ SubStr(Dtos(MAILTO->ZM_EMISSAO),7,2) + "/" + SubStr(Dtos(MAILTO->ZM_EMISSAO),5,2) + "/" + SubStr(Dtos(MAILTO->ZM_EMISSAO),3,2) , Alltrim(MAILTO->ZM_TO), MAILTO->ZM_FILIAL,cErrorMsg, AllTrim(MAILTO->ZM_CHAVE)})
			...
			If lOk .and. !Empty(MAILTO->ZM_ATTACH)	// se enviou e-mail, matamos o anexo - Magal em 05/11/2013
				...
				If ALLTRIM(MAILTO->ZM_BCC) == "CAR"
					...
					For _i:= 1 to Len(aAnexo)
						...
						If File(aAnexo[_i])	 .And. (SubStr(aAnexo[_i],1,13) == "\CAR_TMP\FUR\")

							Ferase(AllTrim(aAnexo[_i]))
				...
				Else
					...
					If File(AllTrim(MAILTO->ZM_ATTACH)) .And. (SubStr(MAILTO->ZM_ATTACH,1,13) == "\CAP_TMP\XML\")
	...
	If !Empty(aExcTent) .Or. !Empty(aExcCAR)
...
Static Function MailTI(aExc)
	...
	Local _i		:= 0
	Local cCorpo	:= ""
	Local cChave	:= "LISTA NAO ENVIADA"
	
	If "CLIENTE" $ aExc[1,5]
	
		cSubject	:= "Composi็ใo de Recebimento - E-mail(s) nใo enviado(s)"
		cMailTo		:= SuperGetMv("AL_MAILCAR"	, NIL	, "")
		cChave		:= "LISTA NAO ENVIADA - CLIENTE"
	...
	If "CLIENTE" $ aExc[1,5]
		...
		cBody    += '             <th width="25%" bgcolor="#999999" scope="col"><span class="style4">Cliente-Lj</span></th>'
		
	Else
		
		cBody    += '             <th width="25%" bgcolor="#999999" scope="col"><span class="style4">Forn-Lj</span></th>'
		
	EndIf

	cBody    += '             <th width="10%" bgcolor="#999999" scope="col"><span class="style4">Emissao</span></th>'
	cBody    += '             <th width="15%" bgcolor="#999999" scope="col"><span class="style4">Para</span></th>'
	cBody    += '             <th width="40%" bgcolor="#999999" scope="col"><span class="style4">Erro</span></th>'
	cBody    += '             </tr>'
	
	For _i := 1 to Len(aExc)   
		
		cBody += '	 	  <tr>'
	
		cCorpo   += 		AllTrim(aExc[_i,3]) + ' - ' + AllTrim(aExc[_i,5]) + ' - ' + AllTrim(aExc[_i,1]) + ' - ' + AllTrim(aExc[_i,2]) + ' - ' + AllTrim(aExc[_i,4]) + CRLF
	
		cBody += '			<td width="10%"><div align="center">' 	+ aExc[_i,3] + '</div></td>' //Filial
		cBody += '			<td width="10%"><div align="center">' 	+ aExc[_i,5] + '</div></td>' //Chave
		cBody += '		 	<td width="10%"><div align="center">' 	+ aExc[_i,1] + '</div></td>' //Emissao
		cBody += '		 	<td width="20%"><div align="left">' 	+ aExc[_i,2] + '</div></td>' //Para
		cBody += '		 	<td width="50%"><div align="left">'  	+ aExc[_i,4] + '</div></td>' //Erro
		cBody += '	   	 </tr>'
	...
	U_EMAILTO(cServer, cAccount, cPass, cFrom, cMailTo, cMailCc, cMailBcc, cSubject, cBody, cFiles, lText,,,.F.,xFilial("SE2"), cCorpo,cChave) 
